...
export class SolicitudComponent implements OnInit {
  ...
  cargarSolicitudes() {
    const id_usuario = this.esAdmin()
      ? 0
      : this.usuarioService.usuario!.Proveedor;
    this.solicitudService
      .obtenerSolicitudes(+id_usuario)
      .subscribe((response) => {
  ...
  resizeGrid() {
    ...
    if (window.innerHeight >= 655) {
      this.grid.height = window.innerHeight * 0.7;
    }
      ...
      .pipe(
        catchError((error) => {
          if (error.status === 400 && error.error && error.error.message) {
            ...
            this.uiService.mostrarAlertaError(
              "Error al registrar solicitud",
              "Ocurrió un error al registrar la solicitud. Por favor, inténtalo de nuevo.",
            );
          }
          return [];
        }),
  ...
  esEstadoAceptado(solicitud: Solicitud): boolean {
    const descripcion =
      solicitud.estado ??
      this.estados.find((e) => e.id_estado === solicitud.id_estado)
        ?.descripcion ??
      "";
  ...
  claseEstado(id_estado: number): string {
    ...
    if (
      descripcion.includes("aprob") ||
      descripcion.includes("acept") ||
      descripcion.includes("autoriz")
    ) {
      return "estado-aprobado";
    }
    if (
      descripcion.includes("rechaz") ||
      descripcion.includes("cancel") ||
      descripcion.includes("denegad")
    ) {
      return "estado-rechazado";
    }
    if (
      descripcion.includes("pendiente") ||
      descripcion.includes("proceso") ||
      descripcion.includes("revis")
    ) {
